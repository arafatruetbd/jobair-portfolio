...
The application root layout SHALL include a skip-to-content link positioned before navigation landmarks that becomes visible on keyboard focus. The system SHALL structure sections using semantic HTML landmarks (`<header>`, `<main>`, `<footer>`, `<section>`). When the user enables reduced motion preferences (`prefers-reduced-motion: reduce`), smooth scrolling and transition effects MUST be disabled.
